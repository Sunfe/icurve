...
IcvTextEdit::IcvTextEdit(QWidget *parent)
    : QMainWindow(parent)
{
...
void IcvTextEdit::closeEvent(QCloseEvent *e)
{
    if (maybeSave())
        e->accept();
    else
...
void IcvTextEdit::setupFileActions()
{
...
void IcvTextEdit::setupEditActions()
{
    QToolBar *tb = new QToolBar(this);
...
void IcvTextEdit::setupTextActions()
{
    QToolBar *tb = new QToolBar(this);
...
bool IcvTextEdit::load(const QString &f)
{
    if (!QFile::exists(f))
        return false;
    QFile file(f);
    ...
    int toPost = textEdit->document()->findBlockByNumber(100).position();
...
void IcvTextEdit::setCurrentFileName(const QString &fileName)
{
...
void IcvTextEdit::filePrintPreview()
{
#ifndef QT_NO_PRINTER
    QPrinter printer(QPrinter::HighResolution);
...
void IcvTextEdit::printPreview(QPrinter *printer)
{
#ifdef QT_NO_PRINTER
    Q_UNUSED(printer);
#else
...
void IcvTextEdit::textUnderline()
{
    QTextCharFormat fmt;
...
void IcvTextEdit::textFamily(const QString &f)
{
    QTextCharFormat fmt;
    fmt.setFontFamily(f);
    mergeFormatOnWordOrSelection(fmt);
...
void IcvTextEdit::textSize(const QString &p)
{
...
void IcvTextEdit::textStyle(int styleIndex)
{
...
void IcvTextEdit::textAlign(QAction *a)
{
...
void IcvTextEdit::currentCharFormatChanged(const QTextCharFormat &format)
{
    fontChanged(format.font());
    colorChanged(format.foreground().color());
}

void IcvTextEdit::cursorPositionChanged()
{
    alignmentChanged(textEdit->alignment());
}

void IcvTextEdit::clipboardDataChanged()
{
#ifndef QT_NO_CLIPBOARD
    if (const QMimeData *md = QApplication::clipboard()->mimeData())
        actionPaste->setEnabled(md->hasText());
...
void IcvTextEdit::mergeFormatOnWordOrSelection(const QTextCharFormat &format)
{
    QTextCursor cursor = textEdit->textCursor();
...
void IcvTextEdit::fontChanged(const QFont &f)
{
...
void IcvTextEdit::colorChanged(const QColor &c)
{
    QPixmap pix(16, 16);
    pix.fill(c);
...
void IcvTextEdit::alignmentChanged(Qt::Alignment a)
{
    if (a & Qt::AlignLeft) {
